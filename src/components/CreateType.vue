...
const createType = async () => {
  ...
  try {
    ...
    if (response.data.is_deleted) {
      toast.add({
        severity: 'success',
        summary: 'Тип операции успешно создана',
      ...
      toast.add({
        severity: 'warn',
        summary: 'Невозможно создать тип операции',
...
<template>
  <Button variant="text" @click="visible = true" size="small">
    <span>Добавить тип операции</span> <CirclePlus />
  </Button>

  <Dialog v-model:visible="visible" modal header="Создать тип операции" class="sm:w-100 w-9/10">
    <div class="inputs">
